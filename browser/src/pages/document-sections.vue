    ...
    <section class="border-b border-slate-200/60 bg-slate-50/30">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
        <div class="flex items-center gap-2 text-xs text-slate-400">
          ...
          <span class="text-slate-600 font-medium">Part {{ partKey }} Sections</span>
        </div>
      </div>
    </section>
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div class="flex items-start gap-4 mb-8">
        ...
        <div>
          ...
          <p class="mt-1 text-sm text-slate-500">{{ partSections.length }} sections found</p>
        </div>
      </div>
      ...
      <div v-if="partSections.length" class="space-y-2">
        <router-link
          ...
          class="group flex items-center gap-4 px-4 py-3 rounded-xl border border-slate-200/60 bg-white hover:border-brand-200 hover:shadow-sm transition-all"
        >
          ...
          <div class="flex-1 min-w-0">
            <span class="text-sm font-medium text-slate-900 group-hover:text-brand-600 transition-colors">{{ section.title || section.filename }}</span>
            <div class="text-[10px] text-slate-400 mt-0.5">{{ section.filename }}</div>
          </div>
      ...
      <div v-else class="py-16 text-center">
        <p class="text-slate-500 text-sm">No document sections found for Part {{ partKey }}</p>
        <p class="text-xs text-slate-400 mt-1">Section data is discovered from AsciiDoc sources at build time.</p>
      </div>
    </section>
  </div>
</template>
