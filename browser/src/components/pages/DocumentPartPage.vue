...
const doc = computed(() => getPartDocument(props.part))
const meta = computed<PartMeta | undefined>(() => {
  const direct = getPartMeta(props.part)
  if (direct) return direct
  // Parent parts like "2" and "11" — synthesize from sections
  const sections = getSectionsForDocument(props.part)
  if (!sections.length) return undefined
  const first = sections[0]
  return {
    domain: props.part === '2' ? 'math' : 'quantities',
    partKey: props.part,
    title: doc.value?.title ?? first.title,
    description: doc.value?.scope ?? '',
    icon: first.icon,
    accent: first.accent,
  }
})
...
const publisher = computed(() => publisherOf(props.part))
...
const allDocs = computed(() => getAllDocuments())
</script>
...
<template>
  ...
  <div v-if="doc && meta">
      ...
      <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20 page-enter">
        <div class="max-w-3xl">
          <div class="flex items-center gap-2 text-xs text-brand-300/60 mb-6">
            ...
            <a href="/documents" class="hover:text-brand-200 transition-colors">Documents</a>
            <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" d="M9 5l7 7-7 7"/></svg>
          </div>
          ...
          <div class="flex items-center gap-3 mb-4">
            ...
            <div>
              <div class="flex items-center gap-2 mb-1">
                ...
                <span class="text-xs text-brand-300/50">{{ doc.edition }}</span>
              </div>
              <h1 class="text-3xl sm:text-4xl font-bold text-white tracking-tight heading-serif leading-tight">
                Part {{ doc.partKey }}: {{ doc.title }}
              </h1>
            </div>
          </div>
          <p class="mt-3 text-sm text-brand-200/60 leading-relaxed">
            {{ publisher === 'IEC' ? 'IEC' : 'ISO' }} 80000-{{ doc.partKey }}
            <template v-if="doc.bilingual"> · Bilingual EN/FR</template>
          </p>
        </div>
      </div>
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
      <div class="max-w-3xl">
        ...
        <p class="text-slate-600 dark:text-slate-400 leading-relaxed text-[15px]">{{ doc.scope }}</p>
      </div>
    </section>
    ...
    <section class="bg-slate-50/50 dark:bg-dark-900/50 border-y border-slate-200/50 dark:border-dark-600/50">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        ...
        <div class="grid sm:grid-cols-2 gap-4 max-w-3xl">
          <div v-for="(h, i) in doc.highlights" :key="i" class="p-4 rounded-xl bg-white dark:bg-dark-800 border border-slate-200/60 dark:border-dark-600/60">
        ...
        <div v-if="doc.note" class="mt-6 p-4 rounded-xl bg-brand-50/40 dark:bg-brand-950/20 border border-brand-200/40 dark:border-brand-800/40 max-w-3xl">
          <p class="text-xs text-brand-700/80 dark:text-brand-300/80 leading-relaxed">{{ doc.note }}</p>
        </div>
      </div>
    </section>
          ...
          <div class="flex items-start gap-4">
            ...
            <div class="min-w-0 flex-1">
              ...
              <p class="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
                Published by {{ publisher === 'IEC' ? 'the International Electrotechnical Commission (IEC)' : 'the International Organization for Standardization (ISO)' }}.
                Edition {{ doc.edition }}.
                Developed by {{ publisher === 'IEC' ? 'IEC/TC 25' : 'ISO/TC 12' }} — Quantities and units.
              </p>
              <div class="mt-4 flex flex-wrap gap-3">
                <a :href="doc.storeUrl" target="_blank" rel="noopener" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-white text-sm font-semibold transition-colors shadow-sm" :class="publisher === 'IEC' ? 'bg-red-600 hover:bg-red-700' : 'bg-brand-600 hover:bg-brand-700'">
                  {{ publisher === 'IEC' ? 'IEC Webstore' : 'ISO Store' }}
      ...
      <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 justify-between">
        <div>
          ...
          <p class="text-sm text-slate-500 dark:text-slate-400">
            {{ entryCount }} {{ meta.domain === 'math' ? 'symbols' : 'quantities' }} defined in Part {{ doc.partKey }}
          </p>
        </div>
        <a :href="browseUrl" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-brand-600 text-white text-sm font-semibold hover:bg-brand-700 transition-colors shadow-sm shadow-brand-600/25">
          View All Entries
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div class="flex flex-wrap gap-2">
          <a
            v-for="d in allDocs"
            :key="d.partKey"
            :href="`/documents/part-${d.partKey}`"
            class="px-3 py-1.5 rounded-lg text-xs font-medium transition-colors"
            :class="d.partKey === props.part
  ...
  <div v-else class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
    ...
    <a href="/about" class="text-brand-600 dark:text-brand-400 text-sm font-medium hover:text-brand-700 dark:hover:text-brand-300">Back to Documents</a>
  </div>
  </div>
</template>
