...
import DimensionOntologyPanel from '../DimensionOntologyPanel.vue'
import { dimensionUrns } from '../../data/urn'

const props = defineProps<{ part?: string }>()
const slug = computed(() => props.part ?? (typeof window !== 'undefined' ? window.location.pathname.split('/').pop() || '' : ''))
...
<template>
  <div>
        ...
        <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
          <div class="max-w-3xl page-enter">
            <div class="flex items-center gap-2 text-xs text-indigo-300/60 mb-5">
              ...
              <a href="/dimensions" class="hover:text-indigo-200 transition-colors">Dimensions</a>
              <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" d="M9 5l7 7-7 7"/></svg>
            </div>
    ...
    <div v-else class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
      ...
      <a href="/dimensions" class="mt-6 inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-brand-600 text-white text-sm font-medium hover:bg-brand-700 transition-colors">
        View all dimensions
      </a>
    </div>
  </div>
