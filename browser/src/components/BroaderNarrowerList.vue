...
<template>
  <div v-if="broader.length > 0 || narrower.length > 0" class="space-y-4">
    <div v-if="broader.length > 0">
      <h4 class="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-2">Broader concepts</h4>
      <div class="flex flex-wrap gap-1.5">
        ...
        >
          <span class="text-[9px]">{{ b.isKind ? '◇' : '↑' }}</span>
          {{ b.label }}
        </a>
      </div>
    </div>
    <div v-if="narrower.length > 0">
      <h4 class="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-2">Narrower concepts</h4>
      <div class="flex flex-wrap gap-1.5">
        <a
          v-for="n in narrower"
          :key="n.id"
