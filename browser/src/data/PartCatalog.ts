...
export const PartCatalog = {
  ...
  partsByDomainMap,
  resolveBasePartKey,
  getPartDocument(partKey: string): PartDocument | undefined {
    return documentMap.get(partKey)
  },
  getAllDocuments(): readonly PartDocument[] {
  ...
  getDomain(key: Domain): DomainInfo | undefined {
    return DOMAINS.find(d => d.key === key)
  },
  getPartMeta(partKey: PartKey): PartMeta | undefined {
    return partMap.get(resolveBasePartKey(partKey))
  },
  getAllParts(): PartMeta[] {
    return [...PARTS]
  },
  getPartsByDomain(domain: Domain): PartMeta[] {
    return partsByDomainMap.get(domain) ?? []
  },
  getSectionsForDocument(docKey: string): PartSection[] {
